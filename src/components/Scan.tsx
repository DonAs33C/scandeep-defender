...
export default function Scan() {
  ...
  const browse = async () => {
    try {
      ...
      if (typeof result === "string" && result) {
        ...
        setReport(null); setError(null); setDuplicate(null);
  ...
  const startScan = async () => {
    ...
    setScanning(true); setError(null); setSteps([]); setReport(null);
    try {
    ...
    try {
      const result = await invoke<ScanReport>("scan_file", { filePath, services: selected });
      setReport(result);
    } catch (e) {
    ...
    <div>
      <div className="card">
        ...
        >
          ...
          <div>{filePath ? filename : tr.drop}</div>       {/* ✅ era tr.dropHint */}
          {filePath && <div style={{fontSize:".78rem",color:"var(--text-muted)",marginTop:".3rem"}}>{filePath}</div>}
        </div>
        {duplicate && (
          <div className="dup-warning">⚠️ {tr.dupTitle}: <strong>{duplicate}</strong></div>
        )}
      </div>
      ...
      <div className="card">
        ...
        <button
          ...
          style={{minWidth:140}}
        >
          {scanning ? <><span className="spinner"/>&nbsp;{tr.scanning}</> : tr.scan} {/* ✅ era tr.analyze */}
        </button>
      </div>
      ...
      {report && (
        <div className="card">
          ...
          <div className="hash-box">{tr.hash}: {report.hash}</div>
